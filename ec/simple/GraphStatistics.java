...
public class GraphStatistics extends SimpleShortStatistics {
	...
	public void postEvaluationStatistics(EvolutionState state) {
		...
		int subpops = state.population.subpops.size();// length; // number of supopulations
		...
		for (int x = 0; x < subpops; x++) {
			for (int y = 0; y < state.population.subpops.get(x).individuals.size(); y++) {
				if (state.population.subpops.get(x).individuals.get(y).evaluated) // he's got a valid fitness
				{
					...
					// update fitness
					if (bestOfGeneration[x] == null || state.population.subpops.get(x).individuals.get(y).fitness
