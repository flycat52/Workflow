...
public class SimpleEvaluator extends Evaluator {
	...
	public void setup(final EvolutionState state, final Parameter base) {
		...
		if (!cloneProblem && (state.breedthreads > 1)) // uh oh, this can't be right
			state.output.fatal("The Evaluator is not cloning its Problem, but you have more than one thread.",
		...
		if (!state.parameters.exists(base.push(P_CHUNK_SIZE), null)) {
			...
			if (chunkSize == 0) // uh oh
				state.output.fatal("Chunk Size must be either an integer >= 1 or 'auto'", base.push(P_CHUNK_SIZE),
						null);
		}
		...
		for (int i = 0; i < pop.subpops.size(); ++i) {
			for (int j = 0; j < state.population.subpops.get(i).individuals.size(); ++j) {
				for (int k = 0; k < numTests; ++k) {
					pop.subpops.get(i).individuals
							.add((Individual) state.population.subpops.get(i).individuals.get(j).clone());
				}
			}
		}
	...
	// Take the N copies of the original individuals and fold their fitnesses back
	// into the original
	// individuals, replacing them with the original individuals in the process. See
	// expand(...)
	...
	/**
	 * A simple evaluator that doesn't do any coevolutionary evaluation. Basically
	 * it applies evaluation pipelines, one per thread, to various subchunks of a
	 * new population.
	 */
	...
	public String runComplete(final EvolutionState state) {
		for (int x = 0; x < state.population.subpops.size(); x++)
			for (int y = 0; y < state.population.subpops.get(x).individuals.size(); y++)
				if (state.population.subpops.get(x).individuals.get(y).fitness.isIdealFitness())
...
//		double min_time = 0;
		for (int pop = 0; pop < len; pop++) {
			...
			// get max values
			ArrayList<Double> totalcost_list = new ArrayList<Double>();
			ArrayList<Double> totaltime_list = new ArrayList<Double>();
			GPIndividual gpInd = null;

			for (int x = fp; x < upperbound; x++) {
				gpInd = (GPIndividual) inds.get(x);
				totalcost_list.add(gpInd.getTotalCost());
				totaltime_list.add(gpInd.getTotalTime());
			}
			Collections.sort(totalcost_list);
			double min_cost = totalcost_list.get(0);
			Collections.reverse(totalcost_list);
			double max_cost = totalcost_list.get(0);
...
//			System.out.println("maxcost = " + max_cost);
//			System.out.println("mincost = " + min_cost);
//			System.out.println("maxtime = " + max_time);
//			System.out.println("mintime = " + min_time);

			for (int x = fp; x < upperbound; x++) {
				gpInd = (GPIndividual) inds.get(x);

				double nor_cost = (max_cost - gpInd.getTotalCost()) / (max_cost - min_cost);
				double nor_makespan = (max_time - gpInd.getTotalTime()) / (max_time - min_time);
				// System.out.println("nor_cost = " + nor_cost);
				// System.out.println("nor_makespan = " + nor_makespan);
				double fitness = 0.1 * nor_cost + 0.9 * nor_makespan;
	...
	// computes the chunk size if 'auto' is set. This may be different depending on
	// the subpopulation,
	...
	int computeChunkSizeForSubpopulation(EvolutionState state, int subpop, int threadnum) {
		...
		// we will have some extra individuals. We distribute these among the early
		// subpopulations
		int individualsPerThread = state.population.subpops.get(subpop).individuals.size() / numThreads; // integer
																											// division
	...
	class SimpleEvaluatorThread implements Runnable {
		...
		public void run() {
			...
			while (true) {
				// We need to grab the information about the next chunk we're responsible for.
				// This stays longer
				...
				synchronized (lock) {
					...
					if (individualCounter >= subpops.get(subPopCounter).individuals.size()) // try again, jump to next
																							// subpop
					{
						individualCounter = 0;
						subPopCounter++;
